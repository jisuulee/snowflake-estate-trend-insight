...
function getFgiColor(fgi) {
  if (fgi >= 1600) return "#d73027";
  if (fgi >= 1300) return "#fc8d59";
  if (fgi >= 1000) return "#fee08b";
  if (fgi >= 800) return "#91bfdb";
  return "#4575b4";
}
...
function renderKoreaMap(geoJsonData, mapData) {
  const mapDataByRegion = {};

  mapData.forEach((item) => {
    const normalizedRegion = normalizeRegionName(item.region);
    mapDataByRegion[normalizedRegion] = {
      ...item,
      region: normalizedRegion,
    };
  });

  console.log("정규화된 mapDataByRegion:", mapDataByRegion);

  if (!koreaMap) {
    koreaMap = L.map("koreaMap").setView([36.35, 127.8], 7);
  ...
  geoJsonLayer = L.geoJSON(geoJsonData, {
    style: function (feature) {
      const regionName = normalizeRegionName(getRegionName(feature));
      const item = mapDataByRegion[regionName];

      return {
        fillColor: item ? getFgiColor(item.fgi) : "#d9d9d9",
        weight: 1,
        opacity: 1,
        color: "#555",
        fillOpacity: 0.75,
    ...
    onEachFeature: function (feature, layer) {
      ...
      console.log("지도 지역명:", regionName, "매칭 데이터:", item);

      if (item) {
        layer.bindTooltip(
          `${regionName}<br>FGI: ${item.fgi}<br>계약 변화율: ${item.contract_change_rate}%`,
          { sticky: true },
        );
        ...
        layer.bindPopup(`
          <b>${regionName}</b><br/>
          FGI: ${item.fgi}<br/>
          계약 변화율: ${item.contract_change_rate}%<br/>
          계약 추이: ${item.contract_trend}
        `);

        layer.on("click", function () {
          const detailCard = document.getElementById("regionDetailCard");
          const detailRegion = document.getElementById("detailRegion");
          const detailFgi = document.getElementById("detailFgi");
          const detailContractChange = document.getElementById(
            "detailContractChange",
          );
          const detailContractTrend = document.getElementById(
            "detailContractTrend",
          );

          if (detailCard) detailCard.style.display = "block";
          if (detailRegion) detailRegion.textContent = regionName;
          if (detailFgi) detailFgi.textContent = item.fgi;
          if (detailContractChange) {
            detailContractChange.textContent = item.contract_change_rate + "%";
          }
          if (detailContractTrend) {
            detailContractTrend.textContent = item.contract_trend;
          }
        });
      } else {
        layer.bindTooltip(`${regionName}<br>데이터 없음`, { sticky: true });
        layer.bindPopup(`<b>${regionName}</b><br/>데이터 없음`);
      }
    },
  }).addTo(koreaMap);
...
async function loadTables() {
  try {
    ...
    document.getElementById("exploreTable").innerHTML = buildOptgroups(
      data.grouped,
    );
...
async function runInsights() {
  ...
  try {
    ...
    if (data.price_top10_down)
      renderHbarChart(
        "priceDownChart",
        data.price_top10_down,
        "rgba(255,107,107,0.75)",
        "집값 변화율 (%)",
      );

    if (data.contract_top10_up)
      renderHbarChart(
        "contractUpChart",
        data.contract_top10_up,
        "rgba(78,205,196,0.75)",
        "거래량 변화율 (%)",
      );

    if (data.contract_top10_down)
      renderHbarChart(
        "contractDownChart",
        data.contract_top10_down,
        "rgba(255,107,107,0.75)",
        "거래량 변화율 (%)",
      );
    ...
    if (data.map_data) {
      const geoResp = await fetch("/static/korea-sido.geojson");
      if (!geoResp.ok) {
        throw new Error("GeoJSON 파일을 불러오지 못했습니다.");
      }
      const geoJsonData = await geoResp.json();
      console.log("geojson 첫 feature:", geoJsonData.features?.[0]);
      renderKoreaMap(geoJsonData, data.map_data);
    }
  } catch (e) {
    console.error("runInsights 에러:", e);
    errorEl.textContent = "분석 실패: " + (e.message || e);
    errorEl.style.display = "block";
  } finally {
    btn.disabled = false;
      ...
      plugins: {
        ...
        tooltip: {
          callbacks: {
            afterLabel: (ctx) => {
              const v = ctx.raw;
              return v >= 60 ? "탐욕 구간" : v >= 40 ? "중립" : "공포 구간";
            },
          },
        },
      },
...
document.querySelectorAll(".tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    ...
    if (tab.dataset.tab === "insights" && koreaMap) {
      setTimeout(() => koreaMap.invalidateSize(), 100);
    }
  });
});
...
async function loadFilters(tableSelectId, stateSelectId, citySelectId) {
  ...
  const citySelect = citySelectId
    ? document.getElementById(citySelectId)
    : null;
...
document.getElementById("predictState").addEventListener("change", () => {
  ...
  if (
    state &&
    filtersCache.cities_by_state &&
    filtersCache.cities_by_state[state]
  ) {
...
function renderScatterChart(chartData) {
  ...
  const points = chartData.actual.map((a, i) => ({
    x: a,
    y: chartData.predicted[i],
  }));
